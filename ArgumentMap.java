...
public class ArgumentMap {
	...
	public void parse(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			if (isFlag(args[i])) {
				flag = args[i];
				this.map.put(flag, null);
			} else if (isValue(args[i])) {
				if (flag != null) {
					this.map.put(flag, args[i]);
					flag = null;
				}
			}
		}
	...
	public static boolean isFlag(String arg) {
		...
		if (arg.length() >= 2) {
			if (arg.charAt(0) == '-' && Character.isLetter(arg.charAt(1))) {
				return true;
			}
		}
	...
	public Path getPath(String flag, Path defaultValue) {
		return this.map.get(flag) == null ? defaultValue : Path.of(this.map.get(flag));
	}
